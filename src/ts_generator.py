...
class ThermalDataGenerator:
    ...
    def duration_generator(self, law: ProbilityLaw, volatility: float, a: float, b: float, expec: int) -> int:
        """
    ...
    def generate_time_series(self, cluster: ThermalCluster, number_of_timeseries: int, output_series: List[List[float]], output_outages: Optional[List[List[Tuple[int]]]] = None) -> None:
        """
        ...
        for ts_index in range(number_of_timeseries + 2):
            ...
            for day in range(self.days_per_year):
                ...
                if PPO != 0 or MXO != 0:
                    true_POD = self.duration_generator(cluster.po_law, cluster.po_volatility, self.ap[day], self.bp[day], cluster.po_duration[day])
                if PFO != 0 or MXO != 0:
                    true_FOD = self.duration_generator(cluster.fo_law, cluster.fo_volatility, self.af[day], self.bf[day], cluster.fo_duration[day])
                ...
                if ts_index > 1: #drop the 2 first generated timeseries
                    ...
                    for h in range(24):
                        output_serie[hour] = cur_nb_AU * cluster.nominal_power * cluster.modulation[h]
                        hour += 1
